...
# ==========================================
# ⚙️ QUOTEX AUTO-TRADE CONFIGURATION
# ==========================================
...
# ব্যবহারকারীর অরিজিনাল Quotex কুকি (লগইন সেশন)
QUOTEX_COOKIE = "activeAccount=demo; laravel_session=%3D; remember_web_59ba36addc2b2f9401580f014c7f58ea4e30989d=%3D%3D"
...
# ==========================================
# 🔌 QUOTEX WEBSOCKET AUTO-EXECUTOR
# ==========================================
async def execute_quotex_trade(direction, amount):
    ...
    try:
        async with ClientSession(headers=headers) as session:
            async with session.ws_connect(url, timeout=5) as ws:
                # 1. Auth Payload
                auth_msg = f'42["authorization",{{"session":"{QUOTEX_COOKIE}","isDemo":1}}]'
                await ws.send_str(auth_msg)
                await asyncio.sleep(0.5)
                
                # 2. Trade Payload
                dir_code = 1 if "BUY" in direction else -1
                trade_msg = f'42["call",{{"asset":"{ASSET_NAME.replace("/", "")}","amount":{amount},"time":60,"direction":{dir_code},"isDemo":1}}]'
                await ws.send_str(trade_msg)
                
                print(f"✅ [QUOTEX EXECUTION] Success -> {direction} ${amount}")
                return True
...
async def live_ai_master_engine():
    ...
    async with ClientSession() as session:
        ...
        await send_telegram(f"🚀 <b>RAFI BHAI AI — QUOTEX AUTO TRADER LIVE</b> 🚀\n\n🌍 <b>Asset:</b> {ASSET_NAME}\n💰 <b>Steps:</b> $2 ➔ $4 ➔ $8 ➔ $18 ➔ $40\n🎯 <b>Target:</b> +${TARGET_PROFIT} | <b>SL:</b> -${STOP_LOSS}\n🛡️ <i>Quotex Server Connected!</i>")
